...
from builders.export_utils import (
    write_exports,
...
def main():
    ...
    builder.register(
        "metric-methods", "pages/metric_methods.html", metric_methods_context
    )
    ...
    for detail_ctx in contradiction_detail_contexts(weights):
        ...
        builder.register(
            page_name, "pages/contradiction_detail.html", lambda c=detail_ctx: c
        )
    builder.register(
        "recommendations",
    ...
    for detail_ctx in recommendation_detail_contexts():
        page_name = detail_ctx["output_filename"].replace(".html", "")
        builder.register(
            page_name, "pages/recommendation_detail.html", lambda c=detail_ctx: c
        )
    ...
    for detail_ctx in plans_detail_contexts():
        page_name = detail_ctx["output_filename"].replace(".html", "")
        builder.register(page_name, "pages/plans_detail.html", lambda c=detail_ctx: c)
    # Audience pages
    builder.register(
        "audience-policymakers",
        "pages/audience_policymakers.html",
        policymakers_context,
    )
    builder.register("audience-lpas", "pages/audience_lpas.html", lpas_context)
    builder.register(
        "audience-developers", "pages/audience_developers.html", developers_context
    )
    ...
    # Write exports
    exports_data = {
        "contradiction-register": read_csv(
            ROOT / "data/issues/contradiction-register.csv"
        ),
        "recommendations": read_csv(ROOT / "data/issues/recommendations.csv"),
        "recommendation_evidence_links": read_csv(
            ROOT / "data/evidence/recommendation_evidence_links.csv"
        ),
        "official_baseline_metrics": read_csv(
            ROOT / "data/evidence/official_baseline_metrics.csv"
        ),
        "implementation-roadmap": read_csv(
            ROOT / "data/issues/implementation-roadmap.csv"
        ),
        ...
        "lpa-quarterly-trends": read_csv(
            ROOT / "data/evidence/lpa-quarterly-trends.csv"
        ),
